...
*   **Python (`clients/python/pi_client.py`):** A client-side script running on devices (like Raspberry Pi) responsible for:
...
## Integrating Local LLMs (Ollama) & RAG

This section outlines how to make your machines intelligent by giving them a "conscious brain" that can reason based on their own experiences.

### Setup on the Raspberry Pi:

1.  **Install Ollama:** Follow the official instructions to install Ollama on your Raspberry Pi.
    ```bash
    # Example for ARM64 (Raspberry Pi 64-bit OS)
    curl -fsSL https://ollama.com/install.sh | sh
    ```
2.  **Pull an LLM Model:** Pull a small, efficient model suitable for the Raspberry Pi (e.g., `phi3`, `llama3:8b-instruct-q4_K_M`).
    ```bash
    ollama run phi3
    # or
    ollama run llama3:8b-instruct-q4_K_M
    ```
    *Ensure Ollama is running (`ollama serve`) when the Python client needs to use it.*
3.  **Install Python Libraries for Ollama:**
    ```bash
    pip install ollama
    ```

### Modifying `clients/python/pi_client.py`:

We will add functions to read from the local data log and send queries to Ollama.
